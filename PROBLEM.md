...
- [x] **specコマンドが低レベルすぎて使えない（node/edgeの抽象化漏れ）** ✅ **解決済み (2026-02-14)**
  - **発見日**: 2026-02-14
  ...
  - **解決内容** (高レベルコマンド実装):
    - ✅ **ユースケース指向コマンド**:
      - `spec add "content"` (Session 34) - 自動kind推論、自動関係推論、UUID不要
      - `spec check` (Session 58) - 矛盾・漏れの統合チェック、終了コード対応
      - `spec find "query"` (Session 67) - 層フィルタリング、意味的検索
      - `spec trace <id>` (Session 58) - 階層的関係表示、深さ制限
      - `spec extract <path>` (Session 93, 97) - コード/protoから自動抽出
      - `spec construct-u0` (Session 55) - U0構築（逆写像エンジン）
      - `spec deprecate/archive/activate <id>` (Session 136-139) - Lifecycle管理
      - `spec export-dot` (Session 129) - グラフ可視化
    - ✅ **ユーザビリティ改善**:
      - UUID管理不要（ID前方一致検索、自動推論）
      - Edge管理不要（自動接続、auto-infer）
      - Kind理解不要（自動推論、デフォルト値）
      - 人間が読める出力（層ラベル、構造化、ページネーション）
      - standaloneモード（サーバー不要、即座に応答）
    - ✅ **低レベルAPI共存**:
      - `spec api list-nodes` - 詳細検査用
      - `spec api get-node` - デバッグ用
      - `spec api list-edges` - 関係分析用
      - 高度なユーザー向けに提供（後方互換性維持）
  - **達成事項**:
    - 初心者: 高レベルコマンドのみで完結（`add`, `check`, `find`, `trace`）
    - 上級者: 低レベルAPIも利用可能（詳細分析、デバッグ）
    - 両立: 同一CLIで両方のレベルをサポート
  - **残課題** (優先度低、非ブロッカー):
    - ⏳ 低レベルコマンドの`spec api`名前空間完全移行
      - 理由: 後方互換性維持（既存スクリプト・ドキュメント）
      - 影響: なし（高レベルコマンドで十分）
  - **解決状況**: ✅ **完了** - 仕様管理の本質機能を高レベルコマンドで実現
...
- [ ] **コードと仕様の双方向同期ができない** ⏳ **将来機能（優先度低）**
  - **発見日**: 2026-02-14
  - **詳細**: `extract`コマンドでコードから仕様を抽出できるが、一方向のみ。コードが変更されたら仕様を更新、仕様が変更されたらコードを更新、という双方向の同期ができない。
  - **現状** (一方向のみ実装):
    - ✅ **Code → Spec** (抽出): `spec extract` で実装済み
      - `metadata.inferred=true` で自動抽出を識別
      - べき等性達成（重複作成しない）
      - 60個の仕様を自動抽出（24.6%）
    - ❌ **Spec → Code** (生成): 未実装
    - ❌ **Bidirectional sync**: 未実装
  - **課題**:
    - **Single Source of Truth問題**:
      - 手動追加した仕様 vs 自動抽出した仕様
      - コード変更時の仕様更新
      - 仕様変更時のコード更新
      - どちらが正か？両方が正か？
    - **競合解決**:
      ...
      - マージすべきか？警告すべきか？
    - **複雑性**:
      - Spec → Code生成は複雑（言語依存、スタイル、既存コードとの統合）
      - コード生成の品質保証が困難
  - **設計上の判断** (意図的に未実装):
    - specORACLEは**観察ベースの抽出**に特化（Beyond-DSL paradigm）
    - 双方向同期は**別ツールの領域**と判断
    - 例: テストフレームワーク、コード生成ツール、IDEプラグイン
  - **現実的なワークフロー** (現状で可能):
    ```bash
    # 1. 定期的に再抽出
    spec extract spec-core/src/ --min-confidence 0.8

    # 2. 差分確認
    git diff .spec/nodes/

    # 3. 手動仕様と競合しないことを確認
    spec check  # 矛盾・孤立をチェック

    # 4. コミット
    git commit -m "Re-extract specs from updated code"
    ```
  - **将来の拡張可能性**:
    - ⏳ `spec extract --diff` - 前回との差分のみ表示
    - ⏳ `spec extract --update-only` - 既存仕様のみ更新（新規追加しない）
    - ⏳ `spec generate-test <id>` - 仕様からテストコード生成（PoC）
    - ⏳ IDEプラグイン - コード変更時に仕様更新を提案
  - **影響範囲**: 高度なワークフローには制限があるが、基本的な使用には影響なし
  - **解決状況**: ⏳ **将来機能** - 基本機能は完成、高度な同期は将来の拡張
...
- [x] **古い仕様を識別できない** ✅ **解決済み (2026-02-15, Git-based versioning)**
  - **発見日**: 2026-02-14
  - **詳細**: 仕様には`created_at`があるが、「この仕様は古くて無効」「これは最新でアクティブ」という区別ができない。
  - **解決内容**:
    - ✅ **Lifecycle status** (Session 136-139実装済み):
      - `active`: 現在有効な仕様（デフォルト）
      - `deprecated`: 非推奨（警告表示、新バージョンが存在）
      - `archived`: アーカイブ（履歴参照のみ、チェックから除外）
    - ✅ **Git-based versioning** (docs/versioning.md):
      - Git commitで変更履歴を管理
      - Commit messageで関係性を記録（Deprecates: <id>, Supersedes with: <id>）
      - `git log`でwho/when/whyを追跡
      - `git diff`でバージョン間比較
    - ✅ **Workflow**:
      ```bash
      spec deprecate <old-id>  # 古い仕様を非推奨に
      spec add "new content"   # 新しい仕様を追加
      git commit -m "Update: ..."  # 変更理由とIDを記録
      ```
  - **利点**:
    - Lifecyclestatusで現在の状態が明確
    - Gitで完全な変更履歴・監査証跡
    - 明示的バージョン番号不要（Git commitがバージョン）
    - チーム開発・レビューに適した構造
  - **参照**: `docs/versioning.md` - 詳細なワークフロー説明
  - **解決状況**: ✅ **完了** - Git-based versioning + lifecycle status で実用的に解決

- [x] **仕様の変更履歴・バージョン管理がない** ✅ **解決済み (2026-02-15, Git-based versioning)**
  - **発見日**: 2026-02-14
  - **詳細**: 仕様を変更する際、上書きするのか、新規追加するのか不明。変更履歴が残らない。
  - **解決内容** (Git-based versioning approach):
    - ✅ **更新ワークフロー確立**:
      ```bash
      # 1. 古い仕様を検索
      ...
      # 4. Gitでコミット（変更理由を記録）
      git commit -m "Update password length: 8→12 chars
      ...
      ```
    - ✅ **変更履歴管理** (Gitによる):
      - `git log -- .spec/nodes/` - 全体の変更履歴
      - `git log -- .spec/nodes/<id>*.yaml` - 特定仕様の履歴
      - `git blame .spec/nodes/<id>*.yaml` - 誰が変更したか
      - `git diff <commit1> <commit2>` - バージョン間差分
    - ✅ **レビュー・承認プロセス**:
      - GitHubプルリクエストで仕様変更をレビュー
      - YAMLファイルのdiffが読みやすい
      - Commit messageで変更理由を確認
      - CI/CDで`spec check`を実行（矛盾検出）
    - ✅ **ロールバック**:
      - `git revert <commit>` - 変更を取り消し
      - `git checkout <commit> -- .spec/nodes/` - 特定時点に戻す
      - Lifecycle status で柔軟に対応（reactivate可能）
  - **利点**:
    - Git = 業界標準の変更管理システム（新しい概念不要）
    - 完全な監査証跡（改ざん不可能なGit history）
    - チーム開発対応（merge, conflict resolution, review）
    - ツールの複雑性増加なし（Gitに委譲）
  - **参照**: `docs/versioning.md` - 詳細なワークフローとベストプラクティス
  - **解決状況**: ✅ **完了** - Git-basedアプローチで実用的に実現

- [x] **仕様の「更新」をどう判断するか不明確** ✅ **解決済み (2026-02-15, Explicit workflow)**
  - **発見日**: 2026-02-14
  - **詳細**: `spec update`のような更新コマンドを実装しようとしても、「どれを更新すべきか」「更新なのか新規なのか」を判断できない。
  - **解決内容** (明示的ワークフロー):
    - ✅ **「更新」の定義を明確化**:
      - specORACLEは**自動判定しない**（意図を推測しない）
      - ユーザーが**明示的に指定**する（deprecate + add + commit message）
      - これは機能不足ではなく、**設計思想**（曖昧な自動判定を避ける）
    - ✅ **更新ワークフロー** (docs/versioning.md):
      ```bash
      # 1. 検索で対象を特定
      spec find "password" --kind constraint
      # → [U0] [a1b2c3d4] Password must be at least 8 characters

      # 2. 人間が判断: これは「更新」か「新規追加」か？
      # → 更新だと判断した場合:
    ...
    - ✅ **アイデンティティモデル**:
      - **UUID** = 技術的ID（システムが生成）
      - **Content + Kind + Layer** = 意味的ID（人間が理解）
      - **Lifecycle status** = 現在の状態（active/deprecated/archived）
      - **Git history** = 変更履歴（誰がいつなぜ変更したか）
    - ✅ **明示的 > 暗黙的**:
      - 自動的な「類似検出→置き換え提案」は実装しない
      - 理由: 誤判定リスク、ユーザーの意図を尊重
      - 代わりに: 検索→判断→明示的操作のワークフロー
  - **利点**:
    - 意図の明確性（自動推測による誤操作を防ぐ）
    - Git commit messageで変更理由が明確
    - 柔軟性（同じ内容でも「新規」「更新」を選べる）
    - シンプル（複雑な名前/スラグ管理不要）
  - **将来の拡張可能性**:
    - ⏳ `spec add`で類似検出→確認プロンプト（オプトイン機能）
    - ⏳ `metadata.name`フィールドで名前ベース管理（要設計）
    - ⏳ `spec replace <old-id> "new content"`ショートカット（要設計）
  - **参照**: `docs/versioning.md` - 更新ワークフローの詳細
  - **解決状況**: ✅ **完了** - 明示的ワークフローで解決（自動判定は意図的に実装しない）

### High

- [x] **spec-cliが継ぎ足し実装の集合になっており、体系化された操作モデルとHuman Friendlyな体験が崩壊している** ✅ **解決済み (2026-02-15, Session 114-116)**
  - **発見日**: 2026-02-14
  ...
  - **解決内容** (Session 114-116):
    - ✅ **モジュール分割完了** (Session 114):
      - `spec-cli/src/commands/` - 20個のコマンドモジュールに分離
      - `spec-cli/src/persistence/` - ストレージ抽象化
      - 各コマンドがユースケース別に独立したモジュールとして実装
    - ✅ **dispatcher抽出** (Session 116):
      - `spec-cli/src/commands/dispatcher.rs` (~300 lines) - コマンド振り分けロジック
      - `main.rs`: 2172行 → 531行 (75.6%削減)
      - CLI境界とアプリケーション境界の明確な分離
    - ✅ **仕様適合確認**:
      - `spec check`: ✅ 0 contradictions, 0 isolated specs
      - main.rs: 531行 (<1000行制約を満たす)
      - [d26341fb]: CLI architecture now meets separation requirement
  - **成果**:
    - **責務分離達成**: CLIパース(main.rs) / ディスパッチ(dispatcher.rs) / ユースケース(commands/) / 永続化(persistence/)
    - **保守性向上**: 新規コマンド追加時は対応モジュールのみ変更
    - **自己統制の実証**: specORACLE自身が自己の違反を検出し、修正を実現
    - **本質の具現化**: U3(実装) → f₀₃⁻¹ → U0(要求) の逆写像により、実装が要求を満たすことを検証
  - **残課題** (優先度低):
    - UX原則のさらなる明文化（現状は仕様として記録済み）
    - 低レベルコマンドの`spec api`名前空間への移動（後方互換性のため保留）
  - **関連タスク**: `tasks/2026-02-15-session-116-cli-refactoring.md`
  - **解決状況**: ✅ **完了** - 責務分離実現、自己統制達成、specORACLEの本質を実証
...
- [x] **list-nodesが大量の結果を一気に表示する** ✅ **解決済み (2026-02-15, Session 134)**
  - **発見日**: 2026-02-14
  - **詳細**: `spec list-nodes`を実行すると577個のノードが一気に表示される。多すぎて把握できない。
  - **影響範囲**: プロジェクトの仕様全体を把握できない。
  - **解決内容** (2026-02-15):
    - ✅ **デフォルトで要約表示** - 層別・kind別の統計を表示
    - ✅ **`--full`フラグ** - 完全リストを表示（オプトイン）
    - ✅ **ページネーション** - `--limit`と`--offset`パラメータ実装
    - ✅ **ナビゲーションヒント** - "Showing X - Y of Z"、"...and N more (use --offset M)"表示
    - ✅ **フィルタリング** - `--layer <N>`、`--kind <type>`でフィルタ可能
  - **検証結果** (Session 134):
    ```bash
    # デフォルト: 要約表示
    $ spec api list-nodes
    ...
    ```
  - **仕様参照**:
    - [c12f2359] - Pagination specification (--limit, --offset)
    - [11678152] - Default summary display
    - [0728ead9] - --full flag support
    - [0f2a7c7e] - --layer filtering
  - **実装**: `spec-cli/src/commands/api.rs:111-210`
  - **タスク**: `tasks/2026-02-15-session-134-verify-list-nodes-pagination.md`
  - **解決状況**: ✅ **完了** - すべての要件を満たす実装が完了し、動作検証済み
