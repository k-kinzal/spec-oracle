...
Our position is that a complete root specification is rarely writable directly by humans;
instead, root admissibility should be \emph{constructed} from layer projections via inverse images.
For example, one intent ``retrieve A'' appears as different layer statements:
application-level goal, API call, HTTP endpoint, and transport-level behavior.
Cross-layer control therefore needs a root-side comparison basis.
...
External evidence is intentionally a convenience-sample PoC ($n=3$), not a population-level estimate.
...
\paragraph{Practical interpretation of $\Omega$.}
$\Omega$ is not assumed to be fully enumerated in practice; it is an interpretation space
approximated by test/simulation traces, runtime logs, or IR-constrained candidates.
The model requires membership judgments, not full enumeration.

\paragraph{NL entry and IR.}
Free-form natural language is allowed as an input artifact.
Formal checks are attached to an intermediate representation (IR) layer,
and extractors (rule-based or LLM-assisted) are treated as $A(i)$ candidate generators.
IR items keep source spans for traceability to original text.
Extractor correctness is then managed by one-sided adequacy assumptions (RQ5 theory).
...
if $\forall x,\ \mathrm{proj}_j(x)=\mathrm{bind}(\mathrm{proj}_i(x),g)$,
then
\[
...
\end{theorem}
\noindent
Current preimage semantics is must-oriented: $none$ implies non-membership.
Alternative may-oriented handling can be adopted operationally, but changes emptiness diagnostics for meet-side checks.
...
\textbf{Boundary note (RQ5/RQ6):}
this section addresses RQ6 (practice) only.
Adequacy assumptions in RQ5 (theory) are proved over abstract relation $E$;
semantic preservation of the concrete regex extractor is not proved here.
...
\paragraph{Negative example (extraction drift).}
Even in numeric-bound constraints, extraction drift can induce false contradictions.
For SQLite page size, misreading ``65536 bytes'' as ``64 bytes'' yields
an empty requirement interval $[512,64]$, making meet-side consistency fail.
This motivates one-sided adequacy management (RQ5 theory) for practical extractors.
This is a constructed risk demonstration, not a statistical field measurement.
...
\end{itemize}
The value is not the raw theorem count, but freezing the design obligations in \S5
(operator split, same-root coupling, one-sided adequacy, partiality/non-adjunction)
as machine-checkable dependencies.
...
ViewPoints/inconsistency-management lines motivate our distributed-layer problem framing,
and pullback-style view-consistency work motivates a shared semantic comparison basis.
Model-synchronization/BX lines (e.g., TGG-based synchronization) motivate transfer/composition concerns.
On the requirements side, EARS/FRET/ACE and property-pattern work motivate practical NL-to-IR structuring,
while Daikon-style invariant mining motivates alternative sources of layer predicates.
Recent LLM-for-RE surveys motivate treating LLMs as extractor candidates under explicit adequacy assumptions.
LLM-based NL-to-LTL translation studies and interactive NL-to-formal workflows
support this ``candidate generation + explicit adequacy management'' stance.
...
\paragraph{Refined thesis (beyond current mechanization).}
For heterogeneous logics, replace simple union-style integration by colimit-style synthesis over a specification diagram.
Then $U0$ in this paper becomes a homogeneous special case.
Projection maps can be lifted to institution morphism/comorphism-style translations
with explicit satisfaction constraints.
Architecturally, a virtual federation model is preferred over a monolithic physical SUM:
keep source artifacts in their native tools and maintain links/transformations only.
Operationally, consistency maintenance should be delta-driven (CPR-style propagation),
with optional runtime projection guards for safety envelopes.
In this division of labor, LLM/agents are candidate generators (NL$\to$IR, link hypotheses),
while formal kernels enforce adequacy and consistency checks.
...
\item Must-oriented handling of $none$ may induce false-contradiction risk in richer constraints; this is not quantified here.
\item Long-term archival strategy (DOI-fixed artifacts / URL-fallback mirrors) is recommended but not institutionalized in this version.
\item Full mechanization of colimit synthesis, institution-morphism constraints, and delta-driven CPR rules is future work.
\end{enumerate}
...
We additionally provide a refined extension path:
colimit-oriented heterogeneous integration, institution-style projection semantics,
virtual federation architecture, and delta-driven consistency operations.
...
[6] Bashar Nuseibeh, Jeff Kramer, Anthony Finkelstein, ``A Framework for Expressing the Relationships Between Multiple Views in Requirements Specification,'' IEEE TSE, 1994. URL: \url{https://www.cs.toronto.edu/~sme/papers/1994/csrp333.pdf}.\\
[7] Martin Wirsing and Alexander Knapp, ``View Consistency in Software Development,'' RADICAL 2004. URL: \url{https://link.springer.com/chapter/10.1007/978-3-540-24626-8_24}.\\
[8] Yingfei Xiong et al., ``Model Synchronization Based on Triple Graph Grammars,'' SoSyM 2012. URL: \url{https://xiongyingfei.github.io/papers/Sosym12.pdf}.\\
[9] Alistair Mavin et al., ``Easy Approach to Requirements Syntax (EARS),'' RE workshop 2009. URL: \url{https://www.researchgate.net/publication/224079416_Easy_approach_to_requirements_syntax_EARS}.\\
[10] NASA FRET Tutorial, 2022. URL: \url{https://ntrs.nasa.gov/citations/20220009659}.\\
[11] Norbert E. Fuchs et al., \emph{Attempto Controlled English (ACE) Manual}. URL: \url{https://attempto.ifi.uzh.ch/site/pubs/papers/ace3manual.pdf}.\\
[12] Matthew B. Dwyer et al., ``Patterns in Property Specifications for Finite-State Verification,'' ICSE 1999. URL: \url{https://ext.math.umass.edu/~avrunin/papers/dwyer99-icse-patterns.pdf}.\\
[13] Michael D. Ernst et al., ``The Daikon system for dynamic detection of likely invariants,'' SCP 2007. URL: \url{https://web.eecs.umich.edu/~weimerw/2024-481F/readings/daikon-tool-scp2007.pdf}.\\
[14] ``Large Language Models for Requirements Engineering: A Systematic Literature Review,'' arXiv, 2025. URL: \url{https://arxiv.org/abs/2509.11446}.\\
[15] ``Learning from Failures: Translation of Natural Language Requirements into Linear Temporal Logic by LLMs,'' URL: \url{https://pure.ecnu.edu.cn/en/publications/learning-from-failures-translation-of-natural-language-requiremen/}.\\
[16] ``Transforming Natural Language Requirements to Formalism,'' INCOSE Systems Engineering. URL: \url{https://incose.onlinelibrary.wiley.com/doi/10.1002/sys.70023}.\\
[17] ``Bidirectionally tolerating inconsistency: partial bidirectional transformations,'' URL: \url{https://groups.inf.ed.ac.uk/bx/fase14.pdf}.\\
[18] ``Automated formalization of structured natural language requirements,'' Information and Software Technology. URL: \url{https://www.sciencedirect.com/science/article/abs/pii/S0950584921000707}.
