...
- `so-daemon`: The `specd` runtime and owner of the domain model. It owns the gRPC service, ingestion, protobuf conversion, persistence, evidence capture and provenance, the volatile Command/Event Bus and Consumers, best-effort Event sinks, and persisted graph generation. It materializes and persists relationships derived by `so-reason` as semantic specification-to-specification Edges, but does not own their semantic derivation.
